...
The system SHALL publish a calendar sync job for a Task or Habit create, update, or delete only when the owning user has an active `GoogleCalendarLink`, and SHALL NOT publish a sync job for habit check-ins.

#### Scenario: Task created by a user with a calendar link
- **WHEN** a user with an active `GoogleCalendarLink` creates a Task
...
#### Scenario: Task updated when a prior sync exists
- **WHEN** a user with an active `GoogleCalendarLink` updates a Task that already has a `CalendarSync` row
